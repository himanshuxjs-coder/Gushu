...
function isChatRouteOpen() {
  if (typeof window === "undefined") return false;
  const win = window as Window & { __GUSHU_CHAT_OPEN__?: boolean };
  if (typeof win.__GUSHU_CHAT_OPEN__ === "boolean") {
    return win.__GUSHU_CHAT_OPEN__;
  }
  return /^\/app\/c\//.test(window.location.pathname);
}

export function isConversationActive(conversationId: string) {
  if (!conversationId) return false;
  if (isChatRouteOpen()) return true;
