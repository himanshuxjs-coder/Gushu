...
export function CameraCapture({ onCapture, onClose }: CameraCaptureProps) {
    ...
    (facing: "user" | "environment") => {
      const baseVideo = selectedCameraId
        ? {
            ...
            width: { min: 1920, ideal: MAX_CAPTURE_WIDTH },
            height: { min: 1080, ideal: MAX_CAPTURE_HEIGHT },
  ...
  const capturePhoto = async () => {
    ...
    const targetWidth = Math.max(video.videoWidth || 4032, 1920);
    const targetHeight = Math.max(video.videoHeight || 3024, 1080);
